 ...
 * code.
 *
 * While using the MPU:D1 mini library, the entirety of the esp8266 library is
 * available for use, meaning you could manage your own WiFi connectivity with
 * calls to WiFi.begin(), WiFi.connect(), etc. Therefore, this library only
 * wraps the aforementioned two WiFi functions to provide minor additional
 * capbility: MiP manages the ssid, password, and hostname variables and
 * animates MiP's eyes while attempting to establish a WiFi connection.
 *
 ...
 * control logic and behaviors for the MiP robot.
 */
void loop() {
  if (!connectResult)
