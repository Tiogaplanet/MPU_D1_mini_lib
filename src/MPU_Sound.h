...
/**
 * @brief Built-in sound effects, mood sound clips, and inline audio sequence volume commands.
 */
enum MiPSoundIndex : uint8_t {
  ...
  MIP_SOUND_VOLUME_7 = 0xFE            ///< Sound sequence command: Set volume to 7 (maximum).
};
...
enum MiPVolume : uint8_t {
  ...
  MIP_VOLUME_DEFAULT = 0xFF///< Special flag to maintain current volume setting.
};

/**
 * @brief Manages MiP's sound playback, audio sequence building, and volume levels.
 */
class MiP_Sound {
 public:
  /**
  ...
  /**
   * @brief Adds a sound entry (with optional delay and inline volume modification) to the sequence.
   *
   * @details Appends a sound clip to the sequence buffer. If a custom volume is specified that
   * differs from the sequence volume, an inline volume change command is automatically injected.
   * Up to 8 sound entries can be added per sequence.
   *
   * @param sound     Sound index to add.
   * @param delayTime Delay in milliseconds before playing the next sound (0 to 7650 ms, converted to 30ms ticks).
   * @param volume    Inline volume for this sound, or MIP_VOLUME_DEFAULT to maintain active volume.
   */
   ...
   *
   * @details Pads any remaining sequence slots with mute sound codes and transmits the 18-byte
   * play sound command to the MiP robot over UART.
   *
   * @param repeatCount Number of times to repeat the entire sequence (0 = play once).
   */
   ...
   *
   * @details Convenience helper method that internally calls beginList(), addEntryToList(),
   * and playList().
   *
  ...
  /**
   * @brief Sets the MiP speaker volume level and verifies the change.
   *
   * @details Sends the set volume command and reads back the active volume level to confirm success.
   * Automatically retries on communication failure or mismatch.
   *
   ...
   *
   * @details Performs a verified read query with automatic retries on communication failure.
   *
 ...
 private:
  ...
  uint8_t m_playCommand[1 + 18];  // Buffer storing play command and 8 sound entries
  int8_t m_soundIndex;
