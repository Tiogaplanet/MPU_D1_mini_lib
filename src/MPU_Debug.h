/**
 * @file MPU_Debug.h
 * @brief Defines the debug functionality and telnet logging interface for the MiP library.
 *
 * @details This header declares the MiPDebug class and associated macros/constants
 * for multi-level logging, execution profiling, and remote telnet debugging on ESP8266.
 *
...
/**
 * @brief Logs formatted text at ANY debug level (always output if debugging active).
...
/** @brief Maximum client inactivity duration before disconnect (in milliseconds). Default: 1 hour (3600000 ms). Set to 0 to disable. */
...
/** @brief Minimum delay in milliseconds before transmitting buffered client packets. */
 ...
 *
 * @details Inherits from Arduino's Print class to support formatted output methods
 * (print, println, printf) over telnet network connections and optional hardware Serial.
 */
class MiPDebug : public Print {
 public:
   ...
   * @{
   */
  static const uint8_t PROFILER = 0; /**< Used to display execution timing of code sections (profiler). */
  static const uint8_t VERBOSE  = 1; /**< Used to display detailed verbose messages. */
  static const uint8_t DEBUG    = 2; /**< Used to display standard debug messages. */
  static const uint8_t INFO     = 3; /**< Used to display informational status messages. */
  static const uint8_t WARNING  = 4; /**< Used to display warning messages. */
  static const uint8_t ERROR    = 5; /**< Used to display critical error messages. */
  static const uint8_t ANY      = 6; /**< Used to display messages unconditionally at any active level. */
  /** @} */
   ...
   * @param enable true to duplicate output to Serial, false to route to telnet only.
   */
  void setSerialEnabled(bool enable);

  /**
   * @brief Configures whether the telnet server allows remote system reset commands.
   * @param enable true to allow remote ESP8266 reset via telnet command, false to disallow.
   */
  void setResetCmdEnabled(bool enable);

  /**
   * @brief Sets custom project help text displayed when a telnet client sends the help command.
  ...
  /**
   * @brief Configures whether timestamps (in milliseconds) are prepended to log outputs.
   ...
   * @param minTime Minimum elapsed time threshold in milliseconds required to trigger profiler output.
   */
  void showProfiler(bool show, uint32_t minTime = 0);

  /**
   * @brief Configures whether debug severity tags ([INFO], [DEBUG], etc.) are prepended to messages.
   * @param show true to show level tags, false to omit.
   */
  void showDebugLevel(bool show);

  /**
   * @brief Configures whether ANSI color codes are included in telnet log output.
   ...
   * @param millisElapsed Threshold duration in milliseconds between loop iterations.
   */
  void autoProfilerLevel(uint32_t millisElapsed);

  /**
   * @brief Applies a text filter string to debug outputs.
   * @param filter Substring filter; only log lines containing this string will be printed.
   */
   ...
   * @return true if specified level meets or exceeds active threshold; false otherwise.
   */
  bool isActive(uint8_t debugLevel = DEBUG);

  /**
   * @brief Writes a single byte to active debug channels (Stream/Print override).
  ...
  /**
   * @brief Writes a byte buffer to active debug channels (Stream/Print override).
 ...
 protected:
  ...
  uint32_t m_autoLevelProfiler =
      0;                     ///< Automatic change to profiler level if time between handles is greater than n millis
