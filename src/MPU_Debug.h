...
*/
/*
   This header file describes the public API for sending debug messages to a
   telnet client. Written by Samuel Trassare and based on Joao Lopes' original
   RemoteDebug library.
 */
#ifndef MPU_DEBUG_H
#define MPU_DEBUG_H
...
// Define an mechanism for quickly calling the various debug levels provided by
// the system.
#define mDebug(...)                \
  {                                \
...
// Maximum time for inactivity (in milliseconds). Set it to 0 to disable
...
class MiPDebug : public Print {
 public:
  ...
  static const uint8_t PROFILER =
      0;  // Used to show time of execution of pieces of code (profiler).
