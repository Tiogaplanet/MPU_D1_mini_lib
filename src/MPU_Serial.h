/**
 * @file MPU_Serial.h
 * @brief Unified serial transport interface for MiP across ESP8266, ESP32, and AVR Pro Mini.
 *
 * @details This header declares the low-level serial API used for communicating
 * with MiP. It handles raw binary transmission, decodes hex-ASCII response strings,
 * and demultiplexes out-of-band (OOB) events.
 *
...
class MiP_Serial {
 public:
   ...
   * @param[out] responseBuffer     Buffer where received response data from MiP will be written.
   * @param[in]  responseBufferSize Maximum capacity of the response buffer.
   * @param[out] responseLength     Reference receiving the actual number of response bytes written.
 ...
 protected:
  static constexpr uint8_t MIP_REQUEST_DELAY = 8;        ///< Minimum delay (ms) between consecutive requests.
  static constexpr uint8_t MIP_RESPONSE_TIMEOUT = 100;   ///< Time (ms) to wait for a response from MiP before timeout.
  static constexpr bool MIP_EXPECT_NO_RESPONSE = false;  ///< Flag indicating no response is expected.
  static constexpr bool MIP_EXPECT_RESPONSE = true;      ///< Flag indicating a response is expected.

  static constexpr size_t MIP_REQUEST_MAX_LEN = 17 + 1;  ///< Longest request is MIP_CMD_PLAY_SOUND.
  static constexpr size_t MIP_RESPONSE_MAX_LEN = 5 + 1;  ///< Longest response is MIP_CMD_GET_CHEST_LED.
