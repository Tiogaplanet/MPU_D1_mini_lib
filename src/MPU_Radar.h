...
enum MiPRadar : uint8_t {
  ...
  MIP_RADAR_10CM_30CM = 0x02, ///< Obstacle detected between 10cm and 30cm in front of MiP.
  MIP_RADAR_0CM_10CM = 0x03,  ///< Obstacle detected between 0cm and 10cm in front of MiP.
  MIP_RADAR_INVALID = 0xFF    ///< Initialized default state prior to receiving any radar event.
};
...
enum MiPRadarMode : uint8_t {
  MIP_RADAR_DISABLED = 0x00, ///< Both radar tracking and gesture detection modes are disabled.
...
class MiP_Radar {
 public:
  ...
  /**
   * @brief MiP protocol notification byte received when a radar distance update arrives.
   */
   ...
   *
   * @return true if radar mode is enabled (mode equals MIP_RADAR), false otherwise.
   */
  bool isEnabled();
   ...
   *
   * @return MiPRadar Current radar proximity range, or MIP_RADAR_INVALID if no data received yet.
   */
  MiPRadar read();
